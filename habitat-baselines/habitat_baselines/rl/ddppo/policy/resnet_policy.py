...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if fuse_keys is None:
            ...
            print("all preprocess keys:", goal_sensor_keys)
            print("all preprocess keys in observation:",
                  [k for k in fuse_keys if k in goal_sensor_keys])
            fuse_keys = [k for k in fuse_keys if k not in goal_sensor_keys]
            print("other keys in observation:", fuse_keys)
            # "depth"
        self._fuse_keys_1d: List[str] = [
            k for k in fuse_keys if len(observation_space.spaces[k].shape) == 1
        ]
        print("1d data key:", self._fuse_keys_1d)
        ...
        if force_blind_policy:
            ...
            print("we use data", [k for k in fuse_keys if
                                  len(observation_space.spaces[k].shape) == 3])
            use_obs_space = spaces.Dict(
                {
    ...
    def forward(
        ...
        # print("observation keys:", observations.keys())
        x = []
        aux_loss_state = {}
        if not self.is_blind:
