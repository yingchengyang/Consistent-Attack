...
class BaseTrainer:
    ...
    def eval(self) -> None:
        ...
        Returns:
            None
        """
        evaluate_strategy = self.config.habitat_baselines.eval.evaluate_strategy
        print("Our evaluate strategy is:", evaluate_strategy)
        # we set evaluate strategy:
        # 0: natural evaluate
        # 1: UAP
        # 2: Reward UAP
        # 3: Trajectory UAP

        print("2", self.config.habitat_baselines.eval_ckpt_path_dir)
        self._add_preemption_signal_handlers()

        resume_state = load_resume_state(self.config, filename_key="eval")
        print("3", self.config.habitat_baselines.eval_ckpt_path_dir)
        # print(resume_state["config"])
        # if we have run this evaluation code,
        # we will save a resume in the
        # "/data/chengyang/habitat-result/result"
        # i.e., config.habitat_baselines.checkpoint_folder
        # and we will load this
        if resume_state is not None:
            ...
            prev_ckpt_ind = -1
        print("4", self.config.habitat_baselines.eval_ckpt_path_dir)
        ...
        with get_writer(self.config, flush_secs=self.flush_secs) as writer:
            ...
            if (
                ...
                if evaluate_strategy == 0:
                    print("We evaluate the model")
                elif evaluate_strategy == 1:
                    print("We evaluate the model under UAP")
                elif evaluate_strategy == 2:
                    print("We evaluate the model under Reward UAP")
    ...
    def _eval_attacked_checkpoint(
        ...
        print("You donot implement this Function!!")
        raise NotImplementedError
